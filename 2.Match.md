# This is the core section of Track-Rex, correlating 2 in-situ or quasi-in-situ EBSD maps.

Current version aligns with _**mtex-6.0.beta3**_, please make sure the right version of MTEX is deployed.

## Section1: Import data & Reconstruct grains
```
fname1 = input('Path to EBSD1: ')			% user input starting grain id in EBSD1
fname2 = input('Path to EBSD2: ')			% user input starting grain id in EBSD1
...
![1](https://github.com/TrackRex/Track-Rex/assets/161822160/cacc4d3f-814c-4e3c-9687-c6c4979a65d8)
...
ebsd = EBSD.load(fname1,CS,'interface','crc',...										% load ebsd1 data
  'convertEuler2SpatialReferenceFrame');
ebsd2 = EBSD.load(fname2,CS,'interface','crc',...										% load ebsd2 data
  'convertEuler2SpatialReferenceFrame');
...
ebsd2 = rotate(ebsd2,rotation.byAxisAngle(xvector,180*degree));						% rotate data for coordinate system

ebsd2 = rotate(ebsd2,rotation.byAxisAngle(zvector,-2*degree));						% ebsd2 is rotate 2 degree for matching

region6 = [140 -800 1362 735]; condition = inpolygon(ebsd,region6); ebsd = ebsd(condition);		% select region you want to match in ebsd1
region5 = [72 -870 1405 760];  condition = inpolygon(ebsd2,region5); ebsd2 = ebsd2(condition);		% select region you want to match in ebsd2
