First open MATLAB, change current folder to MTEX dictionary, implement command 'startup_mtex'.
...
% Orientation Plots, remove '%' if you want to view IPF map.
% figure
...
[grains1,ebsd1.grainId] = calcGrains(ebsd1,'alpha', 2.2, 'angle',[hagb lagb]);
ebsd1(grains1(grains1.grainSize<=2)) = [];
[grains1,ebsd1.grainId] = calcGrains(ebsd1,'alpha', 2.2, 'angle',[hagb lagb]);

[grains2,ebsd2.grainId] = calcGrains(ebsd2,'alpha', 2.2, 'angle',[hagb lagb]);
